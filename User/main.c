...
struct spi_command_t spi_command;  // data from up to spine

// spi buffers
uint16_t rx_buff[RX_LEN];
uint16_t tx_buff[TX_LEN];

struct CANMessage rxMsg1, rxMsg2;
struct CANMessage txMsg1, txMsg2;
struct CANMessage a1_can, a2_can, h1_can, h2_can, k1_can, k2_can;  // TX
                                                                   // Messages

struct leg_state l1_state, l2_state;
;
struct leg_control l1_control, l2_control;
...
void pack_cmd(struct CANMessage* msg, struct joint_control joint) {
...
void unpack_reply(struct CANMessage msg, struct leg_state* leg) {
...
void sendCMD(void) {
    ...
    if (counter > 100) {
        printf("%.3f %.3f %.3f   %.3f %.3f %.3f\n\r", l1_state.a.p,
               l1_state.h.p, l1_state.k.p, l2_state.a.p, l2_state.h.p,
               l2_state.k.p);
        counter = 0;
    }
...
void Zero(struct CANMessage* msg) {
    msg->data[0] = 0xFF;
    msg->data[1] = 0xFF;
    msg->data[2] = 0xFF;
    msg->data[3] = 0xFF;
...
void EnterMotorMode(struct CANMessage* msg) {
    msg->data[0] = 0xFF;
    msg->data[1] = 0xFF;
    msg->data[2] = 0xFF;
    msg->data[3] = 0xFF;
...
void ZeroMode(struct CANMessage* msg) {
    msg->data[0] = 0xFF;
    msg->data[1] = 0xFF;
    msg->data[2] = 0xFF;
    msg->data[3] = 0xFF;
...
void ExitMotorMode(struct CANMessage* msg) {
    msg->data[0] = 0xFF;
    msg->data[1] = 0xFF;
    msg->data[2] = 0xFF;
    msg->data[3] = 0xFF;
...
uint32_t xor_checksum(uint32_t* data, size_t len) {
    uint32_t t = 0;
    for (int i = 0; i < len; i++) t = t ^ data[i];
    return t;
}
...
void spi_isr(void) {
    ...
    spi_put(tx_buff[0] & 0X00FF);
    spi_get();

    dmaMemSize(SPI_RX_DMA, SPI_RX_DMACH, (uint32_t)rx_buff, RX_LEN * 2);
    dmaMemSize(SPI_TX_DMA, SPI_TX_DMACH, ((uint32_t)tx_buff) + 1,
               TX_LEN * 2 - 1);
    ...
    while(IN_spiCs == 0) {
        if(spi_rxdByteStatus) {
            rx_buff[bytecount] = spi_get();
            bytecount++;
            if(bytecount<TX_LEN) {
                spi_put(tx_buff[bytecount]);
            }
        }
    }
    */
    ...
    // TZ
    // 将spi_command传给l1_control
    control();
    // 将l1_control打包为a1_can
    PackAll();
    // 发送a1_can给CAN
    WriteAll();
...
int softstop_joint(struct joint_state state, struct joint_control* control,
                   float limit_p, float limit_n) {
    /*if((state.p)>=limit_p){
...
void control() {
    ...
    if(((spi_command.flags[0]&0x1)==1)  && (enabled==0)){
        ...
        */
    if ((spi_command.flags[0] == 1) && (enabled == 0)) {
        enabled = 1;
        EnterMotorMode(&a1_can);
        can1.write(&a1_can);
        EnterMotorMode(&a2_can);
        ...
        printf("e\n\r");
        return;
    } else if ((spi_command.flags[0] == 0) && (enabled == 1)) {
        enabled = 0;
        ExitMotorMode(&a1_can);
        can1.write(&a1_can);
        ExitMotorMode(&a2_can);
        ...
        printf("x\n\r");
        return;
    } else if ((spi_command.flags[0] == 0XDEAD) && (enabled == 0)) {
        ZeroMode(&a1_can);
        can1.write(&a1_can);
        ZeroMode(&a2_can);
        can2.write(&a2_can);
    ...
    else*/
    {
        ...
        spi_data.flags[1] = 0;
        spi_data.flags[0] |=
            softstop_joint(l1_state.a, &l1_control.a, A_LIM_P, A_LIM_N);
        spi_data.flags[0] |=
            (softstop_joint(l1_state.h, &l1_control.h, H_LIM_P, H_LIM_N)) << 1;
        // spi_data.flags[0] |= (softstop_joint(l1_state.k, &l1_control.k,
        // K_LIM_P, K_LIM_N))<<2;
        spi_data.flags[1] |=
            softstop_joint(l2_state.a, &l2_control.a, A_LIM_P, A_LIM_N);
        spi_data.flags[1] |=
            (softstop_joint(l2_state.h, &l2_control.h, H_LIM_P, H_LIM_N)) << 1;
        // spi_data.flags[1] |= (softstop_joint(l2_state.k, &l2_control.k,
        // K_LIM_P, K_LIM_N))<<2;
...
void test_control() {
    ...
    for (int i = 0; i < DATA_LEN; i++) tx_buff[i] = ((uint16_t*)(&spi_data))[i];
}
...
int main() {
    ...
    memset(&spi_data, 0, sizeof(struct spi_data_t));
    memset(&spi_command, 0, sizeof(struct spi_command_t));
    ...
    if (!spi_enabled) {
        while ((spi_enabled == 0) && (IN_spiCs == 0)) {
            wait_us(10);
        }
        bsp_spi_init();
        spi_enabled = 1;
    ...
    while (1) {
        ...
        // can2.read(&rxMsg2);
        // unpack_reply(rxMsg2, &l2_state);
        // can1.read(&rxMsg1);                    // read message into Rx
        // message storage unpack_reply(rxMsg1, &l1_state); wait_us(10);
    }
}
